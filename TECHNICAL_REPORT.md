...
### 2.13 Warehouse location structure: Picking Zone / Buffer Zone

Physical stock no longer sits flat in `SWH/Stock` — it's organized into a real address scheme matching the physical warehouse layout (`Aisle Section Level PalletPlace`, e.g. `A 01 0 A`, zero-padded section to keep every code a fixed, unambiguous length), generated as 720 leaf locations (6 aisles × 10 sections × 4 levels × 3 pallet places) under two umbrella parents:

- **`SWH/Stock/Picking Zone`** — the forward pick face. Ground floor (Level 0) addresses live here by default.
- **`SWH/Stock/Buffer Zone`** — bulk/overstock. Levels 1–3 live here by default.

**A location's role is just its parent, nothing more** — promoting a Level 1 address to serve as picking (e.g. when ground floor runs out of room for a product) is a single field edit (reparenting it from Buffer Zone to Picking Zone via **Inventory → Configuration → Locations**), not a rebuild. No per-product dedicated locations, no capacity limits — placement is entirely human-decided, matching how this warehouse actually operates.

Both the **Pick** operation's stock rule (`SWH: Stock → Customers`, and its MTO variant) and the **Receipts** operation's stock rule (`SWH: Stock (Buy)`) were rescoped from the flat `SWH/Stock` to `SWH/Stock/Picking Zone` — receiving now defaults into Picking Zone, and picking only ever searches within it. Buffer Zone stock is therefore **never reserved by the sale/delivery flow at all** (not by the ordinary queue, not by force-reserve, not by a hard lock, not by the native forecast "Reserve" button) — which is what makes the picker's manual Buffer→Picking replenishment transfer always safe: nothing in this design can ever put an order's reservation on Buffer stock in the first place, so there's nothing to release before moving it.

Two important gotchas hit while building this, both worth remembering for any future warehouse restructuring:

1. **Changing `stock.picking.type.default_location_src_id` alone does nothing for automated flows.** That field only supplies the default for a transfer manually created through the UI. The moves a confirmed sale order (or PO) actually generates are driven by `stock.rule` records (the warehouse's routes) — those have their own independent `location_src_id`/`location_dest_id` and must be updated directly, or nothing changes for real traffic.
2. **Existing open moves don't retroactively pick up a rule change.** A move's `location_id`/`location_dest_id` is set once, at creation, from whatever the rule pointed at then. Rescoping a rule only affects moves created afterward — every already-open Pick move (from the ~100 orders generated earlier) and every already-open PO receipt had to be explicitly rewritten to the new Picking Zone location as a one-time retroactive fix.

**Migration of pre-existing on-hand stock**: done as a real, validated internal transfer (unreserve whatever was held at flat `SWH/Stock` → `stock.picking` of type Internal Transfer, `SWH/Stock → SWH/Stock/Picking Zone`, validated → re-run the full reservation sweep), never via raw SQL or a blunt zero-then-recreate. A real transfer preserves full traceability and goes through Odoo's own tested quant/reservation accounting; both raw SQL and zero-and-refill were considered and rejected as riskier — raw SQL specifically because it doesn't correctly maintain `stock.quant.reserved_quantity`, a lesson learned the hard way earlier in this same project (see the incident note below).

**"Total in Warehouse" column** (`stock_move.total_warehouse_qty_available`, shown on the Pick transfer's Operations list): since Picking only searches Picking Zone, a line can legitimately show 0 reserved while the product genuinely exists in Buffer Zone. Without context, that reads as "nothing to be had" rather than "go get it from Buffer first." This column shows the product's true total across the whole warehouse (`product.with_context(location=warehouse.lot_stock_id.id).qty_available`, which naturally includes every descendant location) — a live, non-stored figure — so the picker knows there's something worth fetching, without the slip itself ever exposing Buffer as a distinct source location.

**Native forecast report artifact fixed**: scoping Pick's source to a sub-location (Picking Zone) rather than the warehouse's literal top-level Stock location exposed a genuine edge case in Odoo's own forecast reconciliation (`stock.forecasted_product_product._get_report_lines`). The native code remaps every sub-location's on-hand quantity up to the top-level Stock key when tallying "free stock" (so the true total is still counted correctly there) — but it decrements reserved/consumed quantity keyed by the *move's own* location instead (now Picking Zone), a key that was never populated after the remap. The decrement goes negative there, producing a phantom "Free Stock in Transit" line with a negative quantity and no actual demand attached, exactly canceling an amount already correctly reflected in the real "free stock" line. Filtered out in `stock_forecasted.py`'s `_get_report_data` override: any `in_transit` line with no `move_out` and quantity `<= 0` is dropped — a real, positive "genuinely in motion between locations" case is unaffected.
